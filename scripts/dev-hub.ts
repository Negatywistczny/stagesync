...
function enableTerminalGuard() {
  process.stderr.write = (chunk: any) => {
    if (typeof chunk === 'string' && (chunk.includes('DeprecationWarning') || chunk.includes('ExperimentalWarning'))) {
      return true;
    }
...
async function showLANInfo() {
  ...
  console.log(`   LAN Server API:         http://${selectedIP}:4000/api/health`);
  ...
  try {
    if (isWin) {
      ...
      for (const line of lines) {
        if (line.includes('LISTENING')) {
          ...
          if (pid && !list.some((p) => p.pid === pid)) {
            let name = 'nieznany';
            try {
              const taskOut = execSync(`tasklist /FI "PID eq ${pid}" /FO CSV /NH`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
      ...
      for (const pid of pids) {
        let name = 'nieznany';
        try {
          name = execSync(`ps -p ${pid} -o comm=`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
        } catch {
          // ignore
        }
        list.push({ pid, name, port });
...
async function managePortsAndZombies() {
    ...
    if (confirmKill && !clack.isCancel(confirmKill)) {
      ...
      for (const p of allProcs) {
        try {
          ...
          if (isWin) {
            execSync(`powershell -NoProfile -Command "Stop-Process -Id ${p.pid} -ErrorAction SilentlyContinue"`, { stdio: 'inherit' });
          } else {
        ...
        if (remaining.some((r) => r.pid === p.pid)) {
          clack.log.warn(`Proces PID ${p.pid} nie zareagował na soft kill — wymuszanie zamknięcia (force kill)...`);
          try {
            if (isWin) {
  ...
  const zombieScript = path.join(rootDir, 'apps', 'desktop', 'scripts', 'kill-zombies.mjs');
...
async function runDoctorScan() {
  ...
  if (os.platform() === 'win32') {
    const wv2Key = "HKLM:\\SOFTWARE\\WOW6432Node\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}";
    const wv2User = "HKCU:\\Software\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}";
    try {
      const hasWv2 = execSync(`powershell -NoProfile -Command "Test-Path '${wv2Key}'"`, { encoding: 'utf8' }).trim() === 'True' ||
                     execSync(`powershell -NoProfile -Command "Test-Path '${wv2User}'"`, { encoding: 'utf8' }).trim() === 'True';
      if (hasWv2) {
        clack.log.success('WebView2 Runtime: Obecny');
      } else {
        clack.log.warn('WebView2 Runtime: Brak (wymagany do uruchomienia Tauri na Windows)');
      }
    } catch {
  ...
  else clack.log.warn(`Port :3000: Zajęty przez PID ${p3000[0].pid} (${p3000[0].name})`);

  if (p4000.length === 0) clack.log.success('Port :4000: Wolny');
  else clack.log.warn(`Port :4000: Zajęty przez PID ${p4000[0].pid} (${p4000[0].name})`);
  ...
  else if (envExampleExists) clack.log.warn('Plik .env: Brak (dostępny .env.example)');
  ...
  else clack.log.info('STAGESYNC_DATA_DIR: Nieustawiona (używa domyślnego katalogu domowego)');
}
...
function cleanCache() {
  ...
  s.stop(`Zakończono czyszczenie: usunięto ${cleanedCount} katalogów kompilacji/pamięci podręcznej.`);
  ...
  if (lockedPaths.length > 0) {
    clack.log.warn('Zablokowane katalogi (zamknij działające procesy Vite/Tauri/Node):');
...
async function menuTesting() {
  ...
  if (choice === 'map') {
    ...
    runCommand('pnpm', ['benchmark:record'], { env: { RUN_SMART_TEMPO_BENCHMARK: '1' } });
    await waitReturn();
  } else if (choice === 'fix') {
    clack.note('Uruchamianie automatycznego formatowania i poprawek lintera...');
...
async function menuRelease() {
  ...
  if (choice === 'sync') {
    ...
    const linksOk = runCommand('node', ['scripts/quality/check-docs-links.mjs']);
    ...
    clack.log.info(`👁  Podgląd informacji o wydaniu dla wersji v${pkgVer} (Preview Mode):`);
    console.log('\n--- TYTUŁ WYDANIA ---');
    ...
    const bumpType = await clack.select({
      ...
      options: [
        ...
        { value: 'minor', label: '2. ✨  Nowa Funkcjonalność / Minor (np. 5.4.8 -> 5.5.0)' },
...
async function main() {
  ...
  while (true) {
    clearTerminalScreen();
    clack.intro('🎛   StageSync — Developer Experience Suite (DX Hub)');
    ...
    if (category === 'setup') {
      ...
      if (isWin) {
        runCommand('powershell', ['-ExecutionPolicy', 'Bypass', '-File', '.\\scripts\\setup\\setup.ps1']);
      } else {

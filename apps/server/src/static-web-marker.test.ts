...
describe("injectDesktopShellMarker", () => {
  it("injects marker after opening <head>", () => {
    const out = injectDesktopShellMarker(
      "<!doctype html><head></head><body>x</body>",
    );
    expect(out).toContain(
      '<head><meta name="stagesync-shell" content="desktop" />',
    );
    expect(out).toContain('window.__STAGESYNC_SHELL__="desktop"');
    expect(out.indexOf("__STAGESYNC_SHELL__")).toBeLessThan(
      out.indexOf("</head>"),
    );
  });

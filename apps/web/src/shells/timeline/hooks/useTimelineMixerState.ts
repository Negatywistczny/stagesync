...
export function useTimelineMixerState({
  ...
  const [selectedHwOutputId, setSelectedHwOutputId] = useState<string | null>(null);
  const [busRename, setBusRename] = useState<{ busId: string; name: string } | null>(null);
  const [trackRename, setTrackRename] = useState<{ trackId: string; name: string } | null>(null);
  ...
  const onRemoveAudioTrack = useCallback((trackId: string) => {
    if (!draftProject) return;
  ...
  const onDuplicateAudioTrack = useCallback((trackId: string) => {
    if (!draftProject) return;
    if (draftProject.audioTracks.length >= MAX_AUDIO_TRACKS) {
      setLoadError(`Limit ścieżek audio (${MAX_AUDIO_TRACKS}) osiągnięty`);
      return;
  ...
  const openTrackRename = useCallback((trackId: string) => {
    const name =
  ...
  const onAudioTrackHeaderClick = useCallback((e: React.MouseEvent, trackId: string) => {
  ...
  const onAudioTrackSoloClick = useCallback((e: React.MouseEvent, trackId: string) => {
  ...
  const onAudioTrackMuteClick = useCallback((e: React.MouseEvent, trackId: string) => {
    if (!draftProject) return;
  ...
  const buildChannelStripCallbacks = useCallback((trackId: string): ChannelStripCallbacks => {
    return {
  ...
  const openBusRename = useCallback((busId: string) => {
    const name =
  ...
  const openBusContextMenu = useCallback((busId: string, clientX: number, clientY: number) => {
    setClipSelection(clearSelection());
  ...
  }, [setClipSelection, setTrackSelection, openContextMenu, draftProject, commitDraft]);
  ...
  const onHwGainChange = useCallback((hwOutputId: string, gainDb: number) => {
    if (!draftProject) return;
    commitDraft(
  ...
  const onHwMuteToggle = useCallback((hwOutputId: string) => {
    if (!draftProject) return;
  ...
  const onHwChannelModeChange = useCallback((hwOutputId: string, mode: "mono" | "stereo") => {
    if (!draftProject) return;
    commitDraft(
      updateAudioHardwareOutput(draftProject, hwOutputId, {
        channelMode: mode,
  ...
  const onHwSelect = useCallback((hwOutputId: string, e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest("button, label, input")) {
      return;
    }
    setClipSelection(clearSelection());
  ...
  const onHwContextMenu = useCallback((hwOutputId: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    openHwContextMenu(hwOutputId, e.clientX, e.clientY);
  }, [openHwContextMenu]);

  const buildBusCallbacks = useCallback((busId: string): ChannelStripCallbacks => {
    return {
      onSelect: () => {
        setClipSelection(clearSelection());
        setTrackSelection(clearTrackSelection());

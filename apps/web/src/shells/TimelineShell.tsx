...
import { Button, Select, useContextMenu } from "@stagesync/ui";
import {
  resolveMeterAt,
  resolveTempoAt,
  resolveKeyAt,
...
import { FormaClipButton, TouchNudgeBar } from "./timeline/FormaClipButton.js";
import { TimelineHelp } from "./timeline/TimelineHelp.js";
import { TimelineInspector } from "./timeline/TimelineInspector.js";
import {
  formaSubsectionRows,
  renameFormaClip,
  setCountdownBars,
...
import { ChannelStripControls } from "./timeline/channelStrip/index.js";
import type {
...
export function TimelineShell() {
  ...
  const navigateSelection = useCallback(
    (dir: 1 | -1) => {
      const draft = draftRef.current;
      if (!draft) return;
      const lane = primaryLane(clipSelection) ?? "forma";
      const clips = isAudioSelectionLane(lane)
        ? draft.audioClips.filter((c) => c.trackId === lane)
        : lane === "forma"
      ...
      const currentId = clipSelection.primaryId;
      if (currentId) {
        const idx = sorted.findIndex((c) => c.id === currentId);
        if (idx !== -1) {
          const nextIdx = idx + dir;
          if (nextIdx >= 0 && nextIdx < sorted.length) {
            setClipSelection(selectSingle(sorted[nextIdx]!.id, lane));
          }
          return;
        }
      }
      const at = effectiveLocatorTicksRef.current;
      if (dir === 1) {
        const next = sorted.find((c) => c.startTicks >= at);
        if (next) setClipSelection(selectSingle(next.id, lane));
      } else {
        const prev = [...sorted].reverse().find((c) => c.startTicks <= at);
        if (prev) setClipSelection(selectSingle(prev.id, lane));
      }
    },
    [clipSelection],
  );

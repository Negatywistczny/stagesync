...
function loadAudioBufferFromMp3(mp3Path: string): AudioBuffer {
  ...
  return {
    ...
    getChannelData: (ch: number) =>
      ch === 0 ? floatData : new Float32Array(floatData.length),
  } as unknown as AudioBuffer;
}

async function debugBeats() {
  const files = fs.readdirSync(FIXTURES_DIR);
  const rtfFile = files.find(
    (f) => f.toLowerCase().includes("survive") && f.endsWith(".rtf"),
  )!;
  const mp3File = files.find(
    (f) => f.toLowerCase().includes("survive") && f.endsWith(".mp3"),
  )!;

  const rtfContent = fs.readFileSync(path.join(FIXTURES_DIR, rtfFile), "utf-8");
  const lines = rtfContent
    .split("\n")
    .map((l) =>
      l
        .replace(/\\[a-z0-9]+/gi, "")

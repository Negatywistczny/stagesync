...
/**
 * Loads `{origin}/admin` with Offline-First hybrid (#692):
...
class HostWebActivity : AppCompatActivity() {
    companion object {
        ...
        private const val PREFS = "stagesync_console"
    ...
    private fun maybeCheckForApkUpdate() {
        ...
        ApkUpdateChecker.check(hostOrigin, shellVersion, ShellConfig.APK_FILENAME) hostCheck@{ hostOffer ->
            ...
            // Host same version / no APK on host — still offer GitHub Releases when online.
            val snoozed =

...
Domyślnie `build-apk.sh` uruchamia `prepare-local-host.mjs` (libnode 16 KB–aligned + headers + server/web/seed + `android-boot.mjs`). Wymaga NDK 26 + CMake 3.22.1. Zip: digidem `v18.20.4` (nadpisanie `NODEJS_MOBILE_ZIP_URL`). Sideload: `versionCode` ≥ **50208** (`com.stagesync.console.debug`).

...
- [ ] **5.x — Pitch & FX Busses:** Track Pitch Shift + expanded busses / send-return FX (WebAudio; bez VST in-process)
- [ ] **6.0 filary (po major + scope report):** Input & Live Processing (+ recording + proste edit); Audio Suite (Worklet/WASM); Automation lanes (host Tick Engine SSOT); MIDI Patch Matrix + Standalone VSTi Controller (PC/CC) — wbudowane synthy WebAudio = Later 6.x+; szczegóły / OUT w [ADR 0018](./adr/0018-future-audio-architecture.md)

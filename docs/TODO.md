...
**Decyzje PO ≠ backlog:** [ADR 0015](./adr/0015-daw-reference-and-product-decisions.md) (Logic referencja; multi-out tak; auto-update nie; …).
...
- [ ] **Audio — Pencil @ Logic import:** klik w pustym na ścieżce audio z Pencil → Import / File Browser i wstawienie klipu w dokładnej pozycji Timeline ([ADR 0015](./adr/0015-daw-reference-and-product-decisions.md) / [ADR 0008](./adr/0008-timeline-clip-editing.md))
- [ ] **Locator vs playhead:** pass Logic-like (osobne pojęcia; nie scalać kolorów/IA; scrub/seek = komenda SSOT) — [ADR 0015](./adr/0015-daw-reference-and-product-decisions.md)
- [ ] **Safari scratch (WA-MEM-02):** otwarte do naprawy — nie akceptowany permanent limit ([WebAudio triage](./analysis/inspiracje/audyty-silnik/Audyt-Silnika-Odtwarzania-Audio-WebAudio.triage.md))
...
- [ ] [#674](https://github.com/Negatywistyczny/stagesync/issues/674) **Mobile Client:** PWA (`apps/web`) + lekka powłoka Android (`apps/mobile-client`: keep-screen-on / kiosk / QR / mDNS); dystrybucja `.apk` z serwera / Releases — **bez** Google Play *(kierunek arch. zatwierdzony — ADR 0015)*; pełny `mobile-full` + sidecar później

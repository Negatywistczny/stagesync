...
- **WYMAGANIE:** Opisz algorytm **Relacyjnego Zderzania Źródeł (Text-Anchor Bridging)**:
  ...
  - Jak deterministycznie przeliczać relację *Znak w tekście UG → Słowo → Beat z UltraStara → Pozycja na ścieżce Akordów*.
...
Zapis wartości wysokości dźwięku (*Pitch*) w formacie UltraStar przesunięty jest względem standardu MIDI . Wartość `0` odpowiada nucie $\text{C}_4$ (częstotliwość $261.63\text{ Hz}$, numer MIDI `60`) . Zatem konwersja do bezwzględnego numeru MIDI $M$ przebiega według równania:

$M = P + 60$

gdzie $P$ to surowa wartość całkowita wyekstrahowana z wiersza pliku UltraStar .

Typy zdarzeń nutowych w UltraStar wyznaczane są przez pierwszy znak w wierszu . Znak `:` oznacza nutę regularną (*Regular Note*), wymagającą od wokalisty trafienia w wysokość i czas . Znak `*` definiuje nutę złotą (*Golden Note*), naliczającą zwielokrotnione punkty . Znak `R` oznacza nutę rapowaną (*Rap Note*), która ignoruje wysokość pitch i weryfikuje jedynie rytmikę . Znak `G` reprezentuje złotą nutę rapowaną (*Golden Rap Note*), natomiast `F` oznacza nutę swobodną (*Freestyle Note*), niewymagającą oceny . Znak `-` stanowi separator frazy i wymusza podział wiersza tekstu na ekranie, podczas gdy `E` sygnalizuje znacznik końca pliku .
...
Akordy rozpoznawane są za pomocą parsera wyrażeń regularnych opartego na gramatyce bezkontekstowej, wyłapującego nazwy tonacji, stopnie oraz modyfikatory harmoniczne, np. `C#m7b5/G`, `Fsus4`, `H7`, `N.C.` . Uwagi wykonawcze występują w postaci wyrażeń nawiasowych wewnątrz linii akordowej lub tekstowej, takich jak `(pauza)`, `(strum once)`, `(break)` oraz oznaczenia `N.C.` (*No Chord*) .

### Formaty Pomocnicze: MusicXML, ABC Notation i Standard MIDI Files (.mid)

Formaty strukturalne służą w architekturze StageSync jako bezwzględne źródło prawdy dla siatki taktowej (*Measure Grid*), metrum oraz tonacji .
...
Standard MIDI Files (.mid) to binarny strumień zdarzeń zorganizowany w oparciu o jednostki *Ticks Per Quarter Note* ($PPQ$) . Zdarzenia *Meta Tempo* (`0x51`) definiują czas trwania ćwierćnuty w mikrosekundach, natomiast zdarzenia *Meta Time Signature* (`0x58`) określają metrum i liczbę klików metronomu na takt .
...
Dla zagwarantowania pełnej niezależności operacyjnej w StageSync, proces pozyskiwania danych rozdziela surowe strumienie wejściowe na pięć odrębnych warstw równoległych (*Parallel Tracks*), połączonych wspólnym zegarem osi czasu .
...
Gdy w pliku ChordPro lub Ultimate Guitar występuje sekcja beztekstowa (np. `[Solo]` z sekwencją akordów `Am | F | C | G`), w pliku UltraStar występuje w tym miejscu całkowity brak nut wokalnych, objawiający się przerwą instrumentalną . Kotwiczenie słowne staje się wówczas niemożliwe. Sytuacja ta jest obsługiwana przez deterministyczny algorytm równomiernego rozkładu na siatce taktów (*Default Grid Placement*).
...
export interface ASTMetadata {
  ...
  sourceFilesImported: ('ULTRASTAR' | 'CHORDPRO' | 'PLAIN_CRD' | 'MUSICXML' | 'MIDI')[];
}

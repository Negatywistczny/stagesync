...
| **Targeting wyjść** | Matryca audio (Matrix Routing n x m)  | Track / Bus / Master / Direct HW Out  | Auxiliary Busses / Direct HW Out  | **Master / Bus** (Obecny model) / *HW Out (Wymóg)*  |
...
2. Zachowanie ręcznego przycisku FOH GO Pad przy zatrzymanym odtwarzaniu: Czy wciśnięcie ręcznego wyzwalacza GO Pad w stanie spoczynku transportu (`Idle` / `Stop`) ma uruchamiać wyłącznie wybraną próbkę Cue w trybie fire-and-forget, czy powinno automatycznie startować główny transport utworu na osi czasu ? *(Rekomendacja architektoniczna: Uruchamiać wyłącznie próbkę bez startu głównego transportu, zapobiegając przypadkowemu ruszeniu utworu na scenie)* .

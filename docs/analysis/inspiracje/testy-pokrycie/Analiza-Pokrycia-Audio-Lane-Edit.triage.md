...
|----|--------|-----------|------|--------|
| TST-ALE-01 | `joinAdjacent` — asset/gap/window mismatch | P1 | `fixed` | `audioLaneEdit.test.ts` — assetId/gap rejects |
| TST-ALE-02 | `splitAudioClipAt` edges | P1 | `confirmed` | Split/join happy path; brak start/end/no durationMs |
| TST-ALE-03 | `MAX_AUDIO_BUSSES` → `RangeError` | P1 | `fixed` | `audioLaneEdit.test.ts` — limit 16 |
| TST-ALE-04 | `wouldCreateBusCycle` A→B→C→A | P0 | `fixed` | `audioLaneEdit.test.ts` — A→B→C→A cycle |

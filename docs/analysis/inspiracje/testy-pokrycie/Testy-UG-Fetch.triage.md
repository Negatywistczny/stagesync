...
|----|--------|-----------|------|--------|
| TST-UGF-01 | `extractDataContentJson` — odwrotna kolejność atrybutów | P1 | `fixed` | `ug-fetch.test.ts` — reverse attribute order |
| TST-UGF-02 | Cloudflare / 403 / 404 / timeout | P1 | `fixed` | `ug-fetch.test.ts` — async mocked `fetch` |
| TST-UGF-03 | `resolveUgTabUrl` fallback search | P2 | `fixed` | `ug-fetch.test.ts` — resolves via search |

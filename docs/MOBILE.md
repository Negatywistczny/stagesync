...
Domyślny `./apps/console/scripts/build-apk.sh` pakuje `libnode.so` (arm64-v8a + armeabi-v7a), most `stagesync-host-bridge` oraz `assets/host` (server jak sidecar desktop + web + seed). Dane projektów: katalog aplikacji (`filesDir/stagesync-data`). Silnik Node działa w **osobnym procesie** (`:host`) i wątku z 8 MB sterty — awaria natywna nie zabija launchera; status błędu / śmierci procesu trafia po polsku (logcat: tag `SsLocalHost`). Native MIDI na Androidzie jest niedostępne (serwer startuje z `STAGESYNC_MIDI_BACKEND=none`); mDNS na lokalnym hoście Console jest wyłączone — odkrywanie LAN nadal działa przy połączeniu z innym hostem. Host nasłuchuje na `0.0.0.0:4000` (Admin na pętli zwrotnej `127.0.0.1:4000`).

**Ograniczenie (Android 15+ / 16 KB page size):** oficjalny `libnode` z nodejs-mobile `v18.20.4` ma segmenty ELF wyrównane do 4 KB. Na urządzeniach z stroną pamięci 16 KB załadowanie biblioteki może zakończyć proces `:host` (UI Console zostaje). W takim przypadku użyj hosta LAN albo przebuduj z `NODEJS_MOBILE_ZIP_URL` wskazującym na build z `PT_LOAD` ≥ 16 KB (gdy dostępny). Sprawdzenie wyrównania: `prepare-local-host` wypisuje ostrzeżenie przy align &lt; 16384.

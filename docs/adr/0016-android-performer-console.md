...
8. **Offline-First hybrid UI ([#692](https://github.com/Negatywistyczny/stagesync/issues/692)):** APK bundluje bazowy `apps/web` dist (`assets/www`); cold start przez `WebViewAssetLoader` (local-first, API/WS nadal z hosta). `GET /api/health` niesie `protocolVersion` + `uiHash`. Twardy mismatch protokołu → **Remote Mode** (UI z hosta) **bez** kasowania lokalnego bufora. Nowszy / inny `uiHash` na hoście → **jawny** dialog „Zastosuj nowy interfejs” / „Później” (opcja A); **nigdy** cichy sync UI mid-set i **nigdy** cicha instalacja APK. „Zastosuj” pobiera `GET /downloads/ui-bundle.zip` do `filesDir/ui-cache`. Pełny delta/CacheStorage = follow-up; to **nie** jest auto-update natywnego APK.

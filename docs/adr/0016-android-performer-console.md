...
2. **Powłoka:** Kotlin + Android WebView ładujący `apps/web` z hosta LAN — **zakaz** Capacitor/Cordova jako „magii” opakowującej SPA.
...
5. **Dystrybucja:** sideload + GitHub Releases + `GET /downloads/stagesync-performer.apk` i `…-console.apk` z hosta; QR w Adminie. Brak pliku = 404 / empty-state ([ADR 0011](./0011-ui-parity-behavior.md)). Auto-update w tle = **NIE** ([ADR 0015](./0015-daw-reference-and-product-decisions.md)).
...
- CI release może budować APK (`StageSync-Performer-vX.Y.Z.apk`, `StageSync-Console-vX.Y.Z.apk`) gdy Android SDK dostępne.
...
- Issue [#674](https://github.com/Negatywistyczny/stagesync/issues/674)

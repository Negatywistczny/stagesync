...
- **Health / UI sync:** `GET /api/health` zwraca `protocolVersion`, `uiHash` pełnego interfejsu oraz opcjonalnie `uiHashPerformer` / `uiHashConsole`; `GET /api/ui-manifest` (oraz `?role=…`) listę assetów; `GET /downloads/ui-bundle.zip` pełną paczkę oraz `ui-bundle-performer.zip` / `ui-bundle-console.zip` do jawnej aktualizacji na Performer/Console.

#### 📚 Dokumentacja
- **Mobile:** podręcznik [MOBILE.md](./docs/MOBILE.md) — Performer vs Console (Console = pełny odpowiednik desktopu; lokalny host produkt IN), sideload, QR join vs QR APK, Offline-First UI (dialog „Zastosuj”), macierz HW (bez claim green).

#### 📦 Packaging & Desktop (Tauri / Docker)
- **Android:** sideload APK **StageSync Performer** i **StageSync Console** (Kotlin WebView; bez Google Play) — artefakty release + serwowanie z hosta; po połączeniu jawny dialog aktualizacji APK (**Pobierz i zainstaluj** / **Później**), gdy host ma nowszą wersję i APK jest dostępne — bez cichej aktualizacji w tle. **Offline-First:** APK startuje z lokalnego UI (`assets/www`); Performer bundluje tylko Client, Console — **pełne SPA** (Admin + Timeline + Client; link „Klient” działa offline względem bundla); przy innym hashu roli na hoście jawny dialog **„Zastosuj nowy interfejs”** / **Później** (pobranie `ui-bundle-performer.zip` / `ui-bundle-console.zip`); przy niezgodnym protokole tryb zdalny bez kasowania lokalnego bufora. Build APK: tylko ABI `arm64-v8a` + `armeabi-v7a` (bez x86); release z R8. Launcher: ciemny ekran z oficjalnym wordmarkiem StageSync (znak + Stage/Sync) i nazwą roli (**Performer** / **Console**) obok — jak chrome Admin / Timeline, kartami serwerów z sieci (jedno dotknięcie), kafelkami **Skanuj kod QR** / **Ostatnie serwery** oraz ręcznym adresem na dole; w Console widoczny przycisk **Uruchom lokalny host** (przy braku silnika Node w APK — uczciwy komunikat, bez atrapy sukcesu); **Skanuj kod QR** otwiera podgląd kamery (CameraX + ML Kit), rozpoznaje URL LAN hosta z QR „Dołącz” i łączy jak przy ręcznym adresie (wklejenie bez kamery nadal działa; jasne uzasadnienie uprawnienia Kamera po polsku); ikona aplikacji / PWA to oficjalny znak StageSync (ścieżki timeline + amber playhead); wstecz w sesji idzie po historii strony, a **Zmień serwer** / **Dodaj serwer…** jest w ustawieniach Client / Admin (nie jako pływający przycisk nad WebView).
